...
vi.mock("excalibur", () => {
  ...
  class MockTileMap {}
  return { Actor: MockActor, Vector: MockVector, TileMap: MockTileMap, EasingFunctions: { Linear: "linear" }, Engine: class {} };
});
...
vi.mock("@excaliburjs/plugin-pathfinding", () => {
  class MockExcaliburAStar {
    ...
    astar(_from: any, _to: any, _diag: boolean) { return mockAstarPath; }
    getNodeByIndex(idx: number) { return { id: idx }; }
    duration = 1.5;
  }
  class MockExcaliburGraph {
    ...
    resetGraph() { this.nodes.clear(); }
    shortestPath(_from: any, _to: any) { return mockDijkstraPath; }
    duration = 2.5;
  }
  return {
...
vi.mock("../game", () => ({
  game: {
    clock: { schedule: vi.fn((_cb: any, _delay: any) => 0), clearSchedule: vi.fn() },
    add: vi.fn(),
  },
}));
