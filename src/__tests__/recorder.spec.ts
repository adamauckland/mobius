...
describe("GameRecorder", () => {
  ...
  describe("recording", () => {
    ...
    it("records click events with timestamps relative to start", () => {
      ...
      expect(recording.events[0]).toEqual({ timestamp: 100, targetTileIndex: 5 });
      expect(recording.events[1]).toEqual({ timestamp: 250, targetTileIndex: 10 });
    });
  ...
  describe("replay", () => {
    ...
    it("handles empty recordings", () => {
      const recording: GameRecording = { events: [] };
      recorder.startReplay(recording, vi.fn());
      expect(recorder.isReplaying).toBe(true);
