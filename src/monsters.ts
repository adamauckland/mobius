...
let onPlayerKilled: ((player: Player) => void) | null = null;

export function setOnPlayerKilled(cb: (player: Player) => void) {
  onPlayerKilled = cb;
}
...
export function updateMonsters(delta: number) {
  ...
  for (const monster of monsters) {
    ...
    monster.actor.pos.x =
      monster.startPos.x +
      (monster.endPos.x - monster.startPos.x) * progress;
    monster.actor.pos.y =
      monster.startPos.y +
      (monster.endPos.y - monster.startPos.y) * progress;
