...
// "Browse Packs" button — show packs published to Firebase
...
if (btnBrowsePacks) {
  btnBrowsePacks.addEventListener("click", async () => {
    ...
    `;
    listContainer.innerHTML = '<div style="text-align:center;color:#929fa4;">Loading...</div>';
    overlay.appendChild(listContainer);

    document.body.appendChild(overlay);

    try {
      const { browsePacks, getPackProject } = await import("./levelPacks");
      const packs = await browsePacks();

      listContainer.innerHTML = "";
      if (packs.length === 0) {
        listContainer.innerHTML = '<div style="text-align:center;color:#929fa4;">No packs published yet. Create one in the editor!</div>';
        return;
      }
